...
`UsageLogCreateRequest` and `UsageLogCreateItem` must not include `category`.
...
The server returns `category` in `UsageLogResponse` after classification.

### Category Classification

Valid categories are still:
...
Unknown non-system apps may be classified with OpenAI. Invalid or unavailable AI results must fall back to `ETC`.

### DailyAnalysis Response

`DailyAnalysisResponse` may be generated by OpenAI. The response shape remains unchanged:
